...
// 获取喜欢的数量
let getLikeStar = function(type,uid){
  var _sql = `SELECT * FROM likes WHERE iLike='${type}' AND uid='${uid}' ; `
  return query( _sql )
}
// 获取单篇文章like/dislike总的数量
let getUidLikeLength = function(uid){
  var _sql = `SELECT * FROM likes WHERE uid='${uid}'; `
  return query( _sql )
}
// 更新videos star分数
let updateVideoStar = function(value){
  var _sql = `update videos set star=? WHERE id=?; `
  return query( _sql,value )
}
// 更新likes star分数
let updateLikeStar = function(value){
  var _sql = `update likes set star=? WHERE id=?; `
  return query( _sql,value )
}
// 搜索
